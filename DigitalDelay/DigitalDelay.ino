...
void setup() {
  ...
  if (digitalRead(PROG_PIN) == HIGH) {
    ...
    // At the value of 159 for a timer1 of 62.5kHz we get 50kHz.
...
ISR(TIMER1_COMPA_vect) {
  ...
  // I've already read in the next sample for you and pass it to updateAudio
  // Then the output of that function is sent to PORTD to be output properly.
...
void loop() {
  ...
  // Basically we will not update faster than 10 times a second
  updateControl();
 ...
 */
int M = 16;
int* bufferD = new int [M];
int readPtr_ = 0;
int writePtr_= read_ + 4;
int16_t gain = 0;
// ===== GLOBAL VARIABLES =====
...
void userSetup()
{
   ...
   */
   updateControl();
   // Run updateControl once to ensure our gain value is set for the first
   // samples going in

}
 
int8_t updateAudio(int16_t x)
{

    /*y[n]=x[n]+gy[n-m] <--- corrected here
     *
     */
    int y = gain * bufferD[readPtr_];
    // So reading from the buffer is our y[n-m] bit
    // Multiply with g to apply the gain
    // g is a 10-bit number, buffer is a 10 bit number
    // Multiply 2 10-bit is a 20-bit number, so must bitshift back 10-bit
    
    y = y >> 10;
    // Then add x to complete the above
    y += x;
    // Write y to the buffer
    bufferD[write_] = y;

    writePtr_ = (writePtr_++)%M;
    readPtr_ = (readPtr_++)%M;
